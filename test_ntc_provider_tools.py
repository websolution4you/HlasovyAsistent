...
class ProviderNeutralBookingToolsTests(unittest.TestCase):
    ...
    def test_signed_sip_context_lists_and_cancels_owned_booking(self):
        ...
        self.assertEqual(self.database.last_update["payload"], {"status": "cancelled"})
        self.assertEqual(
            self.database.last_update["filters"],
            {
                "id": "booking-1",
                "tenant_id": "tenant-ntc",
                "user_id": "user-1",
                "status": "confirmed",
                "start_at": self.database.last_update["filters"]["start_at"],
            },
        )
